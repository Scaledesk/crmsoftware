<div id="main-wrapper" class="container">
                    <div class="row">
                        <div class="col-md-8 center">
                            <div class="panel panel-white">
                                <div class="panel-body">
                                                              
                                <center><label><h2>Add Supplier Category</h2></label></center>  
                                <div class="row">
                                    <form id="wizardForm" action="" method="post">
                           
                                         <div class="col-md-10 center">
                                    
                                        <div class="form-group col-md-12">
                                            <label>Category Name</label>
                                            <input type="text" class="form-control" id="Category_Name" name="Category_Name" placeholder="Category Name" required="">
                                        </div>

                                        <div class="form-group col-md-12">
                                        <label for="exampleInputEmail"> Category Description</label>
                                        <textarea class="form-control" placeholder="Category Description" id="Category_Description" name="Category_Description" required="" style="margin: 0px -0.078125px 0px 0px; height: 160px; width: 555px;"></textarea>
                                        </div>   
										<div class="form-group col-md-4 center">
								      <center><input type="submit" class="btn btn-primary btn-block" name="" value="Add"></center> 
									  </div>
									</div> 
                                </form>

                            </div>
                        </div>
                    </div>
                </div>
            </div><!-- Row -->
        </div><!-- Main Wrapper -->